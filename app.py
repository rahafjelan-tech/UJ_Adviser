...
HARDCODED_OPENAI_KEY_RE = re.compile(
    r'openai_api_key_value\s*=\s*"sk-[^"]*"'
)
HARDCODED_TAVILY_KEY_RE = re.compile(
    r'os\.environ\["TAVILY_API_KEY"\]\s*=\s*"tvly-[^"]*"'
)
    ...
    for idx, cell in enumerate(nb.cells):
        if cell.cell_type == "code":
            try:
                ...
                cleaned_code = HARDCODED_OPENAI_KEY_RE.sub(
                    'openai_api_key_value = os.getenv("OPENAI_API_KEY", "").strip()',
                    cleaned_code
                )
                cleaned_code = HARDCODED_TAVILY_KEY_RE.sub(
